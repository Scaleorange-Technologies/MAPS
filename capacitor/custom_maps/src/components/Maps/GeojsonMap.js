...
if (typeof window !== "undefined" && !window._externalLocationHandlerSet) {
    ...
    window.handleExternalLocation = (lat, lng) => {
        console.log("[handleExternalLocation] Called with:", lat, lng);
        window.dispatchEvent(new CustomEvent("externalLocation", { detail: { lat, lng } }));
    };
}
// Component to handle map view centering when user location changes
...
// Component to handle map view adjustment when geojson data changes
...
export default function IndiaMapExplorer() {
    ...
    // Load the links JSON file on component mount
    useEffect(() => {
        setLoading(true);
        fetch('/geojsonLinks.json')
            .then(res => {
                if (!res.ok) {
                    throw new Error('Failed to load geojson links');
                }
                return res.json();
            })
            .then(data => {
            ...
            .catch(err => {
                console.error('Error loading geojson links:', err);
    ...
    useEffect(() => {
        function onExternalLocation(e) {
            console.log("[externalLocation] Event received:", e.detail);
            ...
            if (mapRef.current && mapRef.current.setView) {
                mapRef.current.setView([lat, lng], 16); // Zoom in on location
            }
        }
    ...
    // Fetch geojson data when selections change
    useEffect(() => {
        if (!selectedCategory || !links[selectedCategory]) return;

        // Don't fetch anything if we're in STATES category but no state is selected yet
        if (selectedCategory === 'STATES' && !selectedState) return;

        // Don't fetch anything if no layer is selected
        if (!selectedLayer) return;

        let url;
        if (selectedCategory === 'STATES') {
            // For STATES category, we need both state and layer
            if (!links[selectedCategory][selectedState]) return;
            url = links[selectedCategory][selectedState][selectedLayer];
        } else {
            // For other categories, we just need the layer
        ...
        fetch(url)
            .then(res => {
                if (!res.ok) {
                    throw new Error(`Failed to load geojson data (${res.status})`);
                }
                return res.json();
            })
            .then(data => {
    ...
    const determineCurrentState = (coordinates) => {
        // In a real app, you would use reverse geocoding service here
        // For this example, we'll simulate with a placeholder

        // Example of how you might implement this with a reverse geocoding API:
        // fetch(`https://api.example.com/reverse-geocode?lat=${coordinates[0]}&lng=${coordinates[1]}`)
        //   .then(res => res.json())
        //   .then(data => {
        //     setCurrentState(data.state.toUpperCase());
        //   })
        //   .catch(err => {
        ...
        // In the future, you could use the GeoJSON data to determine the state
        setCurrentState('');
    };
          ...
          /> */}
                    ...
                    {geojsonData && (
                        <GeoJSON
                            ...
                            data={geojsonData}
                            style={() => ({
                                fillColor: selectedCategory === 'STATES' ? '#2196f3' :
                                    selectedCategory === 'DISTRICTS' ? '#4caf50' :
                                        selectedCategory === 'ASSEMBLY_CONSTITUENCIES' ? '#9c27b0' : '#ff9800',
                                weight: 2,
                                opacity: 1,
                                color: '#333',
                                dashArray: '3',
